...
Home Assistant가 필요하지 않습니다. 지원하는 Xiaomi BLE 보고를 네이티브
HomeKit 서비스로 변환합니다. 현재 제품 레지스트리에는
`miaomiaoce.sensor_ht.o2` 온습도 센서와 `k0918.toothbrush.t700i` 전동칫솔이
등록되어 있습니다.
...
지원하는 Xiaomi BLE 장치
          │ MiBeacon / MIoT
          ▼
Xiaomi Gateway 3 → openmiio_agent → 로컬 MQTT → 제품 디코더 → HomeKit 액세서리 → Apple 홈
```

## 지원 기능

- `miaomiaoce.sensor_ht.o2`(`pdid=5860`): 온도, 습도, 배터리 잔량 및
  배터리 부족 상태.
- `k0918.toothbrush.t700i`(`pdid=6032`): 양치 활동을 HomeKit 동작 센서로
  표시, 배터리, 강제 종료 시각 보정 및 30초 종료 유실 watchdog.
- 새 설치 시 30초 검색 창에서 지원 장치를 자동 등록. MAC 주소를 미리 알
  필요가 없습니다.
- 재시작 후에도 장치 식별 정보, HomeKit 페어링 및 마지막 온도·습도·배터리
  측정값 유지. 새 BLE 보고가 도착하기 전에도 `0°C` 대신 저장된 값을 표시합니다.
- 중복·역순 BLE 프레임 제거와 T700i 중복 시작 광고의 안전한 처리.
...
3. 장치의 다음 BLE 광고를 기다립니다. T700i는 양치를 시작할 때 활동
   이벤트를 보냅니다.
...
첫 실행 시 안전한 무작위 페어링 PIN을 만들고 30초 동안 일치하는 장치를
...
검색 창에서 여러 장치를 등록하고, 실행 중 새 지원 장치를 발견하면 레지스트리에
...
구독하고 제품 레지스트리에서 PDID를 확인한 뒤 제품별 디코더와 HomeKit
액세서리 팩토리로 전달합니다. 장치 DID를 확인한 뒤에는 MIoT 속성 업데이트도
허용합니다. 자세한 내용과 샘플 페이로드는
[docs/PROTOCOL.md](docs/PROTOCOL.md), 새 제품 추가 절차는
[docs/ADDING-DEVICES.md](docs/ADDING-DEVICES.md)에 있습니다.

HomeKit은 경량 [`github.com/brutella/hap`](https://github.com/brutella/hap)
라이브러리로 구현합니다. 첫 번째 HomeKit 액세서리는 브리지이며, 각 BLE 장치는 MAC
